    ...
    <div class="flex justify-center flex-col py-6 px-4 md:flex-row md:px-0 md:mr-24 md:ml-24 md:py-6">
        <div class="w-full md:w-1/3 bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg mx-2 md:mx-1 mb-4">
            <div class="sm:min-w-36 p-6 text-gray-900 dark:text-gray-100">
                ...
                <x-input-label for="recherche" value="Recherche par mot clé" class="text font-semibold text-lg text-gray dark:text-gray-200" />
                ...
                <form action="{{ route('show.index') }}" method="get" id="filterForm">
                    ...
                    <input type="date" name="date_from" id="date_from" value="{{ $date_from ?? '' }}" onchange="submitForm()" class="w-full max-w-[40ch] border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" />

                    <!-- filtre par date de fin -->
                    <x-input-label for="date_to" value="à" />
                    <input type="date" name="date_to" id="date_to" value="{{ $date_to ?? '' }}" onchange="submitForm()" class="w-full max-w-[40ch] border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" />

                    <!-- filtre par lieu -->
                    <x-input-label for="location" value="Lieux" />
                    <x-select name="location" :options="$lieux ?? []" :selected="$location ?? null" onchange="submitForm()" class="w-full max-w-[40ch]" />
                </form>
            </div>
        </div>
