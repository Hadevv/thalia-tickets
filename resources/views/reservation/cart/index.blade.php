                                        ...
                                        <div class="flex items-start justify-between">
                                            <div class="flex items-center space-x-10">
                                                <div class="w-24 h-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 relative">
                                                    ...
                                                    <img src="{{ asset('images/' . $reservation->representation_reservations->first()->representationSeat->representation->show->poster_url) }}" alt="{{ $reservation->representation_reservations->first()->representationSeat->representation->show->title }}" class="w-full h-full object-cover object-center">
                                                </div>

                                                <div class="flex flex-col">
                                                    <h3 class="text-lg font-semibold">{{ $reservation->representation_reservations->first()->representationSeat->representation->show->title }}</h3>
                                                    <p class="text-sm text-gray-500">Qté {{ $reservation->representation_reservations->sum('quantity') }}</p>
                                                    <p class="text-sm text-gray-500">{{ $reservation->representation_reservations->first()->representationSeat->representation->schedule instanceof \Carbon\Carbon ? $reservation->representation_reservations->first()->representation->schedule->format('d/m/Y H:i') : '' }}</p>
                                                </div>
                                                <h3 class="mt-2">
                                                    {{ $reservation->representation_reservations->first()->representationSeat->representation->schedule instanceof \Carbon\Carbon ? $reservation->representation_reservations->first()->representation->schedule->format('l d F Y') : $reservation->representation_reservations->first()->representationSeat->representation->schedule }}
                                                    {{-- faire une fonction pour rajouter à l'heure de debut le temps de la pieces --}}
                                                    {{ $reservation->representation_reservations->first()->representationSeat->representation->schedule instanceof \Carbon\Carbon ? $reservation->representation_reservations->first()->representation->schedule->format('H:i') : '' }}
                                                </h3>
                                                <ul class="mt-2">
